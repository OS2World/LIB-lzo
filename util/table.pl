...
while (<>) {
	...
	if (/^\s*LZO\s.*library\s+\(v\s*([\w\.\s]+)\s*\,\s*(\d\d[^\)]+)\)/) {
...
sub stats {
	...
	if ($l !~ /^\|\s*(\S+)\s+(\d+)\s+(\d+)\s+(\d+)\s+([\d\.]+\s+)?([\d\.]+\s+)?([\d\.]+)\s+([\d\.]+)\s*\|/) {
		die $_;
	}

	$n++;

	$x5 = ($2 > 0) ? $4 * 100.0 / $2 : 0.0;
	$x6 = ($2 > 0) ? $4 *   8.0 / $2 : 0.0;
...
| Memory manager   : QEMM 7.04, CWSDPMI 0.90+ (r3)                       |
| Compiler         : djgpp v2 + gcc 2.7.2.1                              |
