...
/* #define LZO_HASH_GZIP */
/* #define LZO_HASH_GZIP_INCREMENTAL */
...
#endif
#if defined(LZO_COLLECT_STATS)
			/* collect info about matches that are too far back */
			if (m_pos != NULL &&
			    m_pos[0] == ip[0] && m_pos[1] == ip[1] && m_pos[2] == ip[2])
		  		if (ip - m_pos > MAX_OFFSET)
				{
					lzo_uint t = MIN_MATCH;
					while (t < MAX_MATCH_SHORT && m_pos[t] == ip[t])
						t++;
					lzo_stats->matches_out_of_range++;
					lzo_stats->match_out_of_range[t]++;
		  			if (ip - m_pos > 2 * MAX_OFFSET)
						lzo_stats->matches_out_of_range_2++;
		  			if (ip - m_pos > 4 * MAX_OFFSET)
						lzo_stats->matches_out_of_range_4++;
				}
#endif
		}
                       ...
                       lzo_voidp wrkmem )
{
	...
	/* sanity check */
	if (!lzo_assert(LZO1A_MEM_COMPRESS >= D_SIZE * lzo_sizeof(lzo_byte *)))
		return LZO_E_ERROR;
